"""
EquationAI — ChromaDB Vector Database Manager
Hybrid semantic search using ChromaDB + SentenceTransformers with Jaccard fallback.
"""
...
logger = logging.getLogger("EquationAI.chroma_manager")

# ═══════════════════════════════════════════════════
#  Seed Equations Database
# ═══════════════════════════════════════════════════
...
UNICODE_TO_LATEX = {
    ...
    "γ": "gamma", "δ": "delta", "ε": "epsilon", "σ": "sigma",
    "μ": "mu", "∞": "infty", "∫": "int", "∂": "partial",
    "∇": "nabla", "±": "pm",
}
...
class ChromaManager:
    """Hybrid search engine using ChromaDB vector search + keyword/Jaccard scoring."""
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning(
                f"Could not load chromadb or sentence_transformers: {str(e)}. "
                f"Falling back to local similarity engine."
            )
            self.use_chromadb = False
    ...
    def _seed_chroma_db(self):
        ...
        logger.info(f"Seeded ChromaDB with {len(SEED_EQUATIONS)} equations.")

    def search(self, query: str, limit: int = 5):
        """Hybrid search combining Jaccard, keyword, and vector similarity."""
    ...
    def add_equation(self, equation: str, category: str, explanation: str, tags: list = None):
        """Index a new equation into the search database."""
        if not tags:
            tags = []
        ...
        if not any(clean_latex(e["equation"]) == clean_latex(equation) for e in SEED_EQUATIONS):
            ...
            if self.use_chromadb and self.collection:
                try:
                    ...
                    logger.info(f"Indexed new equation: {equation[:60]}...")
                except Exception as e:
                    logger.error(f"Failed to index equation in ChromaDB: {str(e)}")
